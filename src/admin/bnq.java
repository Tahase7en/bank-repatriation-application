				...
				cat_tree.addMouseListener(new MouseAdapter() {
					...
					if(s.toString()!="Liste banque") {
						if (!s.isLeaf()) {		
							...
							{
								change.splitPane2.remove(panel3);
								
								panel3.removeAll();
						}
							if(panel2.isDisplayable()) 		{					
								change.splitPane2.remove(panel2);
								panel2.removeAll();
							}
							if(CL2.panel2.isDisplayable()) {
								change.splitPane2.remove(CL2.panel2);	
						...
						if (s.isLeaf() & s.getLevel()==1 ) {
							...
							if(panel.isDisplayable()) { 							
								change.splitPane2.remove(panel);
								panel.removeAll();
							}
